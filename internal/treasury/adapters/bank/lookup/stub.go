...
var mockBanks = func() []domain.BankInfo {
	...
	for _, r := range raw {
		b, err := domain.NewBankInfo(r.name, r.bin, r.code, r.shortName, r.logo, r.logo, 1, 1)
		if err != nil {
...
func (s *StubClient) ListBanks(_ context.Context) ([]domain.BankInfo, error) {
	if s.err != nil {
		return nil, s.err
	}
	return s.banks, nil
...
func (s *StubClient) FindByBankCode(_ context.Context, bankCode string) (domain.BankInfo, error) {
	if s.err != nil {
